...
struct AccelUpdate {
public:
    std::array<double, 3> update = { 0,0,0 };
};
...
double calcForcesOnCell(int c, std::vector<Atom>& atomList, std::ofstream& debug) {
    ...
    std::unordered_map<int, AccelUpdate> accelUpdates;
    ...
    for (auto updatePair : accelUpdates) {
        ...
        for (int k = 0; k < 3; k++) {
            atomList[updatePair.first].accelerations[k] += updatePair.second.update[k];
        }
        mutexes[updatePair.first].unlock();
    }
    return netPotential;
}
...
double calcForces(std::vector<Atom>& atomList, std::ofstream& debug) { // Cell pairs method to calculate forces
    ...
    for (int c = 0; c < numCellsXYZ; c++) {
        potentialArr[c] = pool.submit(calcForcesOnCell, c, std::ref(atomList), std::ref(debug));
    }
    for (int i = 0; i < numCellsXYZ; i++) {
        netPotential += potentialArr[i].get();
    }
    return netPotential;
}
