...
// If using graphing function, g++ celllists.cpp -DMOLDY_PLOT -I/usr/include/python3.10 -lpython3.10 -O2
#endif
...
int main()
{
    ...
    { // Main loop handles integration and printing to files
        ...
        { // Update positions
            for (int j = 0; j < 3; ++j)
            {
                atomList[k].positions[j] += atomList[k].velocities[j] * timeStep + .5 * atomList[k].accelerations[j] * timeStep * timeStep;
        ...
        if (i < numTimeSteps / 2 && i % 5 == 0)
        { // Apply velocity modifications for first half of sample
            thermostat(atomList);
        }

        if (i > numTimeSteps / 2)
        { // Record energies to arrays and file after half of time has passed
    ...
    { // Graph potential, kinetic, and total energy plot
...
void thermostat(std::vector<Atom> &atomList)
{
    ...
    for (int i = 0; i < N; i++)
    { // Add kinetic energy of each molecule to the temperature
...
double calcForces(std::vector<Atom> &atomList, std::ofstream &debug)
{ // Cell pairs method to calculate forces
    ...
    for (int j = 0; j < N; j++)
    { // Set all accelerations in every atom equal to zero
        for (int i = 0; i < 3; ++i)
        {
    ...
    { // Initialize all cells in header array to empty
        header[c] = -1;
    }
    ...
    for (mc[0] = 0; mc[0] < numCellsPerDirection; (mc[0])++)
    { // Calculate coordinates of a cell to work in
